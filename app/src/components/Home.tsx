...
export default class Home extends Component<HomeProps, HomeState> {
  ...
  async componentDidMount(): Promise<void> {
    try {
      const userRequestResponse = await APIHTTPClient.getUser(
        "fcb0cf58-36e0-4d4c-8aa2-f399503eff0b"
      );
      const postsByUserRequestResponse: Array<any> = await APIHTTPClient.getPostsByUser(
        userRequestResponse.id
      );

      const latestPostId = postsByUserRequestResponse[0].id;
      const latestPost = await APIHTTPClient.getPost(latestPostId);
